...
namespace ccFramework {
    ...
    int ccRequest::getRequestParameterAsInt(std::string name, int default_value) {
        ...
        return ccCommon::string2int((std::string) this->getRequestParameterAsArray(name));
    }

    double ccRequest::getRequestParameterAsDouble(std::string name, double default_value) {
        if (!this->hasRequestParameter(name)) return default_value;
        return ccCommon::string2double(this->getRequestParameterAsArray(name));
    }
    ...
    ccArray ccRequest::getRequestParameterAsArray(std::string name, ccArray default_value) {
        if (!this->hasRequestParameter(name)) return default_value;
        return *this->query_params.getByPath(name);
    }

    ccArray ccRequest::getPostParameterAsArray(std::string name, ccArray default_value) {
        if (!this->hasPostParameter(name)) return default_value;
        return *this->post_params.getByPath(name);
    }

    ccArray ccRequest::getRequestParamters() {
        return this->query_params;
    }

    ccArray ccRequest::getPostParamters() {
