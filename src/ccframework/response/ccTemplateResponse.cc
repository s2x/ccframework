...
namespace ccFramework {
    ...
    ccTemplateResponse::ccTemplateResponse(std::string dict_name, std::string template_name) {
        ...
        this->layout = ccApp::getInstance()->getConfigValue("routes." + rname + ".layout",
                                                            ccApp::getInstance()->getConfigValue(
                                                                    "templates.default_template", "layout.tpl"));
        ...
        this->description = ccApp::getInstance()->getConfigValue("routes." + rname + ".description",
                                                                 ccApp::getInstance()->getConfigValue(
                                                                         "templates.default_description", ""));
        this->title = ccApp::getInstance()->getConfigValue("routes." + rname + ".title",
                                                           ccApp::getInstance()->getConfigValue(
                                                                   "templates.default_title", ""));
        this->keywords = ccApp::getInstance()->getConfigValue("routes." + rname + ".keywords",
                                                              ccApp::getInstance()->getConfigValue(
    ...
    ccResponse *ccTemplateResponse::render() {
        ...
        this->layout_dict->SetValue("HEAD_STYLES", this->renderCssList(ccTemplateResponse::default_stylesheets) +
                                                   this->renderCssList(this->stylesheets));
        this->layout_dict->SetValue("HEAD_JAVASCRIPTS",
                                    this->renderJsList(ccTemplateResponse::default_head_javascripts) +
                                    this->renderJsList(this->javascripts_head));
        this->layout_dict->SetValue("FOOTER_JAVASCRIPTS",
                                    this->renderJsList(ccTemplateResponse::default_footer_javascripts) +
                                    this->renderJsList(this->javascripts_foot));
        ...
        ccFramework::ccEventDispatcher::notify("ccTemplate.beforeRenderLayout",
                                               {std::make_pair("template", (void *) this)});
    ...
    void ccFramework::ccTemplateResponse::addCss(std::string uri) {
